...
int main(void)
{
  ...
  while (1) {
	...
	RS485_create_packet(TxData, &packet_size, (uint8_t *)"HOLA!", 0);
	...
	while(!ackRecv){
		...
		if(NewMessage){
			...
			{
				if(ns_rec == ns && ack)	//Comprobamos que el numero de mensaje coincida con el de ack
				{
					ackRecv = 1;
				}
			}
